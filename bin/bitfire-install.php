...
use function TF\tar_read_file as TFTar_read_file;
...
COOKIE => "\$_COOKIE",
COOKIE+1 => "cookie support found. Browser verify available.",
SHMOP => "SHMOP",
...
SUCCESS => "SUCCESS",
SUCCESS+1 => "BitFire installed successfully.  Please visit <a style='color:#506690' target='_blank' id='link' data-href='/?BITFIRE_PAGE=DASHBOARD' href='/?BITFIRE_PAGE=DASHBOARD' title='Launch BitFire Dashboard'>BitFire Dashboard</a><br><br>password: <span style='color:#335EEA;text-shadow:none;cursor:pointer;' onclick='copy_text(this)' id='pass'>%s</span> <small style='font-size:.8rem;color:#555;'>(click to copy to clipboard)</small>",
UNIN => "SUCCESS",
...
REMOVE_SUCCESS => "BitFire startup has been removed from %d",
REMOVE_FAIL => "BitFire startup unable to removed from %d"
));
...
function fin($title, $success = "error", ...$args) {
    ...
    if ($GLOBALS['WAIT']??false) {
        $m .= '<div>Please wait for PHP ini cache to expire before loading...</div><h1 style="margin: 10px auto 10px auto" id="timer" data-sec="'.$GLOBALS['WAIT'].'">00:00</h1><script type="text/javascript">let x=document.getElementById("link");x.title="Please wait for countdown timer before launching Dashboard.";x.href="";x.setAttribute("disabled","disabled");function cl(x){console.log(x);}function cntdwn() {let e=document.getElementById("timer");let ds=parseInt(e.getAttribute("data-sec"));e.setAttribute("data-sec", ds-1);let min=Math.floor(ds/60);let sec=ds-(min*60);if(ds>=0){e.innerText = min + ":" + String(sec).padStart(2,"0");window.setTimeout(cntdwn, 1000);}else{let x=document.getElementById("link");x.title="Launch BitFire Dashboard";x.href=x.getAttribute("data-href");x.removeAttribute("disabled");}}cntdwn();</script>';
    }
...
function tar_read_file($fh, TarHeader $header) {
    ...
    while ($header->size > 0 && $ctr++ < 20000) { // max 10Mb
        ...
        $len = strlen($tmp);
        if ($len != GZBLOCKSZ) { Err::new(READ_FAIL, "1:$len"); fin(FAILED); }
...
function tar_extract(string $file, string $destination = "") {
    ...
    if ($input == false) { Err::new(OPEN_FAIL, $file); fin(FAILED); }

    while(($header = tar_read_header($input, $destination))) {
        if ($header->type == 5) {
            if (!file_exists($header->filename)) {
                if (!mkdir($header->filename, 0775, true)) {
                    Err::new(MKDIR_FAIL, $header->filename); fin(FAILED); 
                }
            }
        }
...
function tar_read_header($fh, string $dest) {
    ...
    if (!$header || ($data['checksum'] > 0 &&$header->checksum != OctDec(trim($data['checksum'])))) {
        Err::new(CHK_FAIL, $header->checksum, $data['checksum']); fin(FAILED);
    }
...
function uninstall() {
	...
	if (strlen($r) < strlen($c)) {
		$success |= file_put_contents($hta, $r, LOCK_EX);
		debug(($success) ? REMOVE_SUCCESS : REMOVE_FAIL, $hta);
	}
	...
	if ($r) {
		if (is_writeable($ini)) {
			$tmpsuc |= file_put_contents($ini, $r, LOCK_EX);
            $success |= $tmpsuc;
	...
	if (strstr(file_get_contents($index), "/bitfire/") != false) {
		$tmpsuc = \TF\file_replace($index, "<?php @include $root/bitfire/startup.php;?>\n", "")->run()->num_errors() > 0;
		$success |= $tmpsuc;
...
\TF\file_replace("$root/bitfire/config.ini", "disabled", $pass)->run();
// update cookie support in config
if (isset($_COOKIE['_bft'])) { \TF\file_replace("$root/bitfire/config.ini", "/cookies_enabled\s*=\s*false/", "cookies_enabled = true")->run(); }
...
if ($apache) {
	if ((file_exists($hta) && is_writeable($hta)) || $writeable) {
		...
		if (!in_array("# BEGIN BitFire", $lines)) {
			...
			copy($hta, "$hta.bak.".mt_rand(10000, 99999));
			$c = file_get_contents($hta);
...
if (!$startup && !empty($ini)) {
	@chmod($ini, FILE_RW2);
	$ini_w = is_writeable($ini) || (!file_exists($ini) && is_writeable(dirname($ini)));
	if ($ini_w) {
		$c = @file_get_contents($ini) || "";
		file_put_contents("$ini.bak.".mt_rand(10000, 99999), $c, LOCK_EX);
		...
		if (preg_match("/(\s*;)\s*auto_prepend_file\s*\=\s*(.*)/", $c, $matches)) {
			$comment = "\n";
			if (!\TF\contains($matches[1], ";") && file_exists($matches[2])) {
				debug("auto_prepend_file", "chained [{$matches[2]}] to BitFire startup");
				\TF\file_replace("$root/bitfire/config.ini", "auto_prepend_file = \"\"", "auto_prepend_file = \"{$matches[2]}\"")->run();
				$comment = "; replaced by bitfire:{$matches[0]}\n";
			} 
			if (\TF\file_replace($ini, "/\s*;?\s*auto_prepend_file\s*=.*/", "{$comment}auto_prepend_file = \"$root/bitfire/startup.php\"")->run()->num_errors() > 0) {
				debug(ADDEDINI);
