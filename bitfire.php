...
class BitFire
{
    ...
    public function inspect() : \TF\Maybe {
        ...
        if (!Config::enabled(CONFIG_ENABLED)) { die("not enabled"); return $block; }

        // don't inspect local commands
        if (!isset($_SERVER['REQUEST_URI'])) { return $block; }
        assert(isset($this->_request[REQUEST_METHOD]), "request method is not set");
        ...
        */
        ...
        if (!$block->empty()) {
            ...
            \BitFire\log_request($request, $block->value(), $ip_data);
        }
    ...
    /**
     * verify that the configuration file contains all necessary configuration
     * @param string $config_file the path to the bitfire configuration file
     * @return array the read configuration data, or null if the config file has errors
     */
    protected function verify_config($config_file) : array {
        $file = $config_file;
        assert(file_exists($file), "ini file not found"); // DELETE_ME

        if (file_exists($file)) {
            $config = parse_ini_file($file, false);
            // verify that required settings are set
            if (isset($config['strict_len']) && 
                isset($config['cache_type']) &&
        ...
        throttle_error(1009);
        return array ('cache_type' => 'file');
    }



    /**
     * if we have too many failed requests from this IP, block it
     */
    protected function update_response_code_blocking($code) {
        $this->_ip_data[$code]++;
        if ($code > 399 && $this->_ip_data[$code] > Config::int('max_errors_before_block')) {
            //$this->add_fail_reason(40001, "http_code", strval($code), "too many http [$code] codes", $code, 1);
        }
    }
    /**
     * helper function to convert an array of parameters into url encoded data
     */
    }
...
function process_request(array $get, array $post, array $server, array $cookie = array()) : array {
    ...
    $request['USER_AGENT'] = $server['HTTP_USER_AGENT'] ?? null;
    $request['SCHEME'] = $server['REQUEST_SCHEME'] ?? 'https';
...
function log_request(array $request, Block $block, array $ip_data) {
    //list($eventId, $name, $item, $match) = $reason;
