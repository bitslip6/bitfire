...
function bit_http_request(string $method, string $url, $data, int $timeout = 5, array $optional_headers = null) {
    ...
    if ($foo === false) {
        $foo = system("curl -X$method --header 'content-Type: '{$optional_headers['Content-Type']}' $url -d '".escapeshellarg($content)."'");
    }
...
function cookie(string $name, string $value, int $exp, bool $secure, bool $httponly) {
    ...
    if (PHP_VERSION_ID < 70300) { 
        setcookie($name, $value, '/; samesite=strict', '', $secure, $httponly);
    } else {
